...
Это warning, не fatal. RAG (поиск по курсу) не подключится, но
базовый Q&A через облачный LLM продолжит работать. Если RAG нужен —
запусти LM Studio с загруженной моделью `bge-m3` (квантованный GGUF).
