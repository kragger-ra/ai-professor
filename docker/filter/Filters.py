    ...
    class Filter:
        ...
        ModelLocalPaths = {
            "judge": {
                ...
                "localPath": "/models/apancJudge",
            },
            "topics": {
                "id": "apanc/russian-sensitive-topics",
                "localPath": "/models/apancTopics",
            },
            "tiny_classificator": {
                "id": "Den4ikAI/ruBert-tiny-replicas-classifier",
                "localPath": "/models/den_tiny_replicas",
            },
            "synonims": {
                "id": "inkoziev/sbert_synonymy",
                "localPath": "/models/kozievSynonims",
            },
        }
        ...
        def CheckModel(self):
            if not self.ModelLoaded:
                ...
                self.FilterJudgeModel = BertForSequenceClassification.from_pretrained(
                    ...
                    cache_dir=self.thisfolder
                    + self.ModelLocalPaths["judge"]["localPath"],
                )
                print(
        ...
        def get_sentence_type(self, text):
            inputs = self.tokenizer_for_classificator(
                text.replace("?", ""),
                max_length=512,
        ...
        def Filter(self, ninp):  # [2.0,2.0,50,100]
            ...
            for i, line in enumerate(inp):
                ...
                preds = adjust_multilabel(
                    model_output["logits"],
                    self.target_vaiables_id2topic_dict,
                    is_pred=True,
                )
                if preds != "none":
...
if __name__ == "__main__":  # DEBUG NOT WORK
    ...
    LargeFREDProc = multiprocessing.Process(
        target=FILTERS_PROCESS, args=(filtersCtx,)
