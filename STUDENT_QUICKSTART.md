...
Tutor автоматически найдёт `samples/vyshmat/` по `course_config.yml` и построит RAG. Никакого пути диктовать не нужно — имя курса берётся из поля `short_name` в конфиге.

Подробнее об образце — [samples/README.md](samples/README.md).

### Свои пакеты в папке `courses/`

Если хочешь добавить **свой курс** и обращаться к нему короткой командой — положи папку курса (вместе с `course_config.yml`) в `courses/` в корне репо. Tutor сканирует `courses/`, `samples/` и `data/courses/` при каждой команде.

Пример:
```
courses/
├── linal/             ← short_name: "Линал"
│   ├── chapter_1.md
│   └── course_config.yml
```
Голосом: «Загрузи курс линал».

Подробнее: [courses/README.md](courses/README.md).
