...
class pycord_voice_client:
    def __init__(
        self, bot, autonomus=False, ctx_chat=None, guild_id=None, docker_sender=None
    ):
        if ctx_chat is None:
            self.ctx_chat = []
        else:
    ...
    def start_recognition_functions(self):
        ...
        async def finished_callback_sendfiles(
            sink, channel: discord.TextChannel, *args
        ):
            ...
            if len(recorded_users) > 0:
                ...
                for user_id, audio in sink.audio_data.items():
                    ...
                    send_filename = (
                        bot.get_user(user_id).display_name
                        + "-"
                        + datetime.datetime.now().strftime("%Mm-%Ss")
                        + f".{sink.encoding}"
                    )
        ...
        async def finished_callback_recognize(
            sink, channel: discord.TextChannel, *args
        ):
        ...
        finished_callback_recognize = finished_callback_sendfiles
        ...
        if self.autonomus:
            ...
            async def on_ready():
                ...
                print(
                    "[PYCORD] Logged in to Discord as {} - ID {}".format(
            ...
            async def vc_listen(ctx):
                ...
                self.debug_text_channel = bot.get_channel(
                    1132660411477544962
                )  # <- open test closed test 1130078872256389120)
                ...
                if sound_found:
                    ...
                    for user, audio in self.sink.audio_data.items():
                        if user not in collected_voice_data:
                            file = io.BytesIO()
                            collected_voice_data.update(
                                {user: discord.sinks.AudioData(file)}
                            )
...
if __name__ == "__main__":
    ...
    stt_class_client_sender = None  # DockerSTTSender()

    bot_voice_class = pycord_voice_client(bot, autonomus=True)
    bot_voice_class.start_recognition_functions()
    # bot_voice_class.
    bot.run(get_secret("DiscordToken"))
