"""
DISCORD SOCIAL SERVICE
from old prototype
not implemented yet
TODO #1: voice support
...
def calcTime(time):
    return (
        bcolors.OKGREEN
        + str((datetime.datetime.now() - time).total_seconds())
        + bcolors.ENDC
    )
...
class MainControlDiscord:
    ...
    def __init__(self, main_ctx, ctxx, ctx_chat, docker_sender=None):
        ...
        self.TOKEN = get_secret("DiscordToken")  # DiscordToken
        self.CPREF = "."
        ...
        self.bot = commands.Bot(
            command_prefix=bot_command_prefixes, intents=intents, activity=activity
        )
            ...
            while True:
                ...
                if firstCheck:
                    ...
                    if len(self.VisoredName[0]) > 2:
                        ...
                        emb.add_field(
                            name="==== " + self.VisoredName[0] + " ====",
                            value="-    начинаем прослушку    -",
                            inline=True,
                        )
                        emb.add_field(
                            name="==== последние 5 логов ====",
                            value="-    выводим последние 5 сообщений    -",
                            inline=False,
                        )
        ...
        async def SendInChatQ(whatto, RemoveTime):
            ...
            if not CheckCtx is None and whatto != "":
                ...
                if not isinstance(
                    whatto, discord.embeds.Embed
                ):  # cutIndex(str(type(whatto)),'embed'):
                    if len(whatto) > 0 and len(whatto) < 1000:
                        ...
                        print(
                            "[Discord] Сообщение не соответствует требованиям. Слишком много или нет знаков."
                        )
                else:
                    if RemoveTime != 0:
                ...
                try:
                    ...
                    if ctxx.ds_actions_q.qsize() > 0:
                        ...
                        if act_type == "stream_start":
                            print(
                                "[DISCORD ACTION WORKER] ПОЛУЧЕН ЗАПРОС НА АНОНС СТРИМА!"
                            )
                            ...
                            hello_text = (
                                mention_text + "НАЧАЛСЯ СТРИМ :fire: :sunglasses: "
                            )
                            emb = discord.Embed(
                                title="ЗАЛЕТАЙ НА СТРИМ =) Ссылочки на Twitch и Ютуб ниже!",
                            ...
                            emb.add_field(
                                name="YouTube", value=self.stream_link, inline=True
                            )
                            emb.add_field(
                                name="Twitch", value=twitch_stream_link, inline=True
                            )
                            ...
                            thread = await channel.create_thread(
                                name="стрим " + timeString,
                                type=ChannelType.public_thread,
                                message=stream_message,
                            )
                            await thread.send(
                                "Сюда можете присылать угарные таймкоды со стрима и комментировать)"
                            )
                        elif act_type == "test_message":
                            print(
                                "[DISCORD ACTION WORKER] ПОЛУЧЕН ЗАПРОС НА ВЫПИСЫВАНИЕ СООБЩЕНИЯ!"
                            )
                            channel = self.bot.get_channel(1132660411477544962)
                            await channel.send(act["msg"])
                        elif act_type == "answered_message":
                            print(
                                "[DISCORD ACTION WORKER] ПОЛУЧЕН ЗАПРОС НА ВЫПИСЫВАНИЕ СООБЩЕНИЯ В ОТВЕТЫ НА СТРИМЕ!"
                            )
        ...
        async def on_voice_state_update(member, prev, cur):
            ...
            if cur.channel == channel:  # в канале для траснляций?
                if not (cur.afk or cur.deaf or cur.mute):
                    if (
                        cur.self_mute and not prev.self_mute
                    ):  # Would work in a push to talk channel
                        print(
                            f"[DISCORD VOICE CHANNEL] {user} stopped talking! (micro off)"
                        )
                    elif prev.self_mute and not cur.self_mute:  # As would this one
                        print(
                            f"[DISCORD VOICE CHANNEL] {user} started talking! (micro on)"
                        )
                    elif not cur.self_mute:
                        print(
                            f"[DISCORD VOICE CHANNEL] {user} started talking! (other, join or smth)"
                        )
        ...
        async def on_message(msg):
            ...
            if msg.content.startswith("хихихи"):
                ...
                answer = "Может, мне тоже посмеяться, {0} ?".format(
                    msg.author.name[0:5] + "что-то там"
                )
                # await msg.reply(answer, delete_after = 15, mention_author=False)#msg.channel.send(answer)
            if msg.content.startswith("ЦифруМне"):
                answer = "{0}, твой DISCORD ID = >{1}<".format(
                    msg.author.name[0:5] + "что-то там", msg.author.id
                )
            for bot_nick in bot_command_prefixes:
                if msg.content.lower().find(bot_nick) != -1:
                    ...
                    if msg.content.find("300") != -1:
                        ...
                        await msg.reply(
                            f"<@{str(msg.author.id)}> услышала вопрос, думаю над ответом...",
                            delete_after=6,
                            mention_author=True,
                        )
                        ctx_chat.append(
                            {
                                "date": eztime(),
                                "user": msg.author.name,
                                "msg": str(cutIndex(msg.content, "ответь ")),
                                "env": "discord",
                                "server": "1122595942986694658",
                                "discord_id": str(msg.author.id),
                                "processing_timestamp": time.time_ns(),
        ...
        async def on_ready():
            print(
                "[DISCORD] МОДУЛЬ Подключен + готов к работе! >",
                GetTimeDelta(timeStartMetrics),
            )
            print(
                "[DISCORD] LOGIN {} - ID {}".format(
        ...
        async def on_command_error(ctx, error):
            fun_osks = [
                ...
                "ты бот",
                "кривожоп",
                "зачитай заклинание че папе че маме",
                "где девственность оставил",
            ]
            await ctx.message.reply(
                f"{random.choice(fun_osks)}: {str(error)}",
                delete_after=6,
        ...
        class PyAudioPCM(discord.AudioSource):
            ...
            def __init__(
                ...
                rate=48000,
                chunk=960,
                input_device=audio_input_device_index,
            ) -> None:
                p = pyaudio.PyAudio()
                print(
                    "[DISCORD VOICE] Коннект:", p.get_device_info_by_index(input_device)
                )
                self.chunks = chunk
        ...
        @self.bot.command(
            aliases=["звук"], pass_context=True
        ...
        async def voice_start(ctx=None, guild=None):  # создаем асинхронную фунцию бота
            ...
            channel = self.bot.get_channel(
                1122595944798625925
            )  # голосовой Комната для траснляций
            ...
            vc.play(
                PyAudioPCM(),
                after=lambda e: (
                    print(f"Ошибка в проигрывании error: {e}") if e else None
                ),
            )
            if not contextless:
                await ctx.message.reply(
                    "Тестирование звука...", delete_after=6, mention_author=False
                )
                await ctx.message.delete()  # чистим чат от твоих команд
        ...
        @self.bot.command(
            aliases=["clear", "чистка", "посудомойка"]
        )  # разрешаем передавать агрументы
        ...
        async def on_errr(ctx, err):
            if isinstance(err, commands.MissingPermissions):
                await ctx.message.reply(
                    "У тя нет прав бездарь тупой пзхвапзва на "
        ...
        async def ahelp(ctx):  # создаем асинхронную фунцию бота
            ...
            emb.add_field(
                name=self.CPREF + "gs", value="глобальное смс ботярам", inline=False
            )
        ...
        @self.bot.command(
            aliases=["stat", "status", "стат", "статус"]
        )  # разрешаем передавать агрументы
        async def statusbots(ctx):  # создаем асинхронную фунцию бота
            print("[DISCORD] запрос на вывод СТАТУСА")
            emb = discord.Embed(
                title=f"Самочувствие NetTyan",
                description="Вывод важных показателей статуса",
            )
            emb.add_field(name=str(main_ctx.mood), value="настроение", inline=True)
            emb.add_field(
                name="[YT ON]" if main_ctx.IsYTChatConnected else "[YT OFF]",
                value="youtube",
                inline=True,
            )
            ingame = main_ctx.ingame
            emb.add_field(
                name="[MC ON]" if ingame else "[MC OFF]", value="minecraft", inline=True
            )
            if ingame:
                emb.add_field(
                    name=str(main_ctx.ingame_info.get("ground_block", "пустота"))
                    + " | "
                    + str(main_ctx.ingame_info.get("held_item", "ничего")),
                    value="блок | вещь",
                    inline=True,
                )
                emb.add_field(
                    name=str(main_ctx.ingame_info.get("task_chain", "нет задач")),
                    value="игровые задачи",
                    inline=False,
                )
            ...
            for k, q in enumerate(ctx_chat):
                ...
                emb = discord.Embed(
                    title=f'#{str(k)} {q.get("user", "??? (UNDEFINED)")}',
                    description=str(q.get("date")),
                )
                emb.add_field(
                    name="Сказал:", value=q.get("msg", "(ничего)"), inline=False
                )
                ...
                if filt_allowed is not None:
                    ...
                    if sentence_type:
                        emb.add_field(
                            name="Тип сообщения:", value=sentence_type, inline=True
                        )
        ...
        @self.bot.command(
            aliases=["гс", "gs"], pass_context=True
        )  # разрешаем передавать агрументы
        ####ДЛЯ РАЗРЕШЕНИЙ ТОЛЬКО АДМИНАМ:
        ####commands.has_permissins (administrator = True)
        ...
        @self.bot.command(
            aliases=["пр", "ch", "check", "чек"], pass_context=True
        )  # разрешаем передавать агрументы
        ####ДЛЯ РАЗРЕШЕНИЙ ТОЛЬКО АДМИНАМ:
        ####commands.has_permissins (administrator = True)
        async def CheckMsgs(ctx, arg):  # создаем асинхронную фунцию бота
            if True:
                ...
                if len(self.VisoredName[0]) > 2:
                    ...
                    CheckStartMsg = await ctx.message.reply(
                        "Чекаем ботяру:\n" + arg + "\n___________", mention_author=False
                    )
                else:
                    await ctx.message.reply(
                        "закончила прослушивать: указан " + arg,
                        delete_after=5,
                        mention_author=False,
                    )
                ...
                await ctx.message.reply(
                    "нет такого: " + arg, delete_after=5, mention_author=False
                )
            await ctx.message.delete()  # чистим чат от твоих команд
        ...
        def RunBot():
            ...
            try:
                if FirstRun:
                    print(
                        "[DISCORD] начинаем запуск.....", GetTimeDelta(timeStartMetrics)
                    )
                    ...
                    if self.docker_sender is not None:
                        # self.recognition_client = pycord_voice_client(
                        #     self.bot,
                        #     autonomus=False,
                        #     ctx_chat=ctx_chat,
                    ...
                    self.bot.run(
                        self.TOKEN
                    )  # , log_handler=None) # log_handler=None отключает Логи дискорда
                    FirstRun = False
            except BaseException as err:
                # print('>'+str(err)+'<')
                while (
                    not SuccesfullRun
                    and str(err)
                    == "Cannot connect to host discord.com:443 ssl:default [getaddrinfo failed]"
                ):
                    try:
...
def DiscordProcess(ctx, main_ctx, ctx_chat, proxy=True):
    print("[DISCORD MODULE] Инициирован запуск ДИСКОРД ПРОЦЕССА")
    t = datetime.datetime.now()
    proxy = get_secret("proxy")
    print("Detected proxy needed for use: " + str(proxy))
    if proxy:
        print("Detected proxy needed for use: " + str(proxy))
...
if __name__ == "__main__":
    ...
    DiscordProcess(main_ctx=ctx, ctx=discordCtx, ctx_chat=ctx_chat)
