...
TrovoClientID = get_secret("TrovoClientID")
TrovoAccessToken = get_secret("TrovoAccessToken")
...
class TrovoClient:
    ...
    def actions_listener_thread_start(self, _trovo_actions_queue):
        self.act_listener_thread = Thread(
            target=self.actions_listener, args=(_trovo_actions_queue,), daemon=True
        )
        ...
        try:
            if data:
                response = requests.post(
                    url=url, headers=self._generate_request_headers(), data=data
                )
            else:
                response = requests.get(
                    url=url, headers=self._generate_request_headers()
                )
            data = response.json()
            if data.get("error", "") != "":
    ...
    def validate_access_token(self) -> bool:
        ...
        if resp and len(str(resp.get("expire_ts", ""))) > 1:
            print(
                "[trovo DEBUG validating token] expire_ts=",
                resp.get("expire_ts", "NO INFO"),
            )
            return True
        return False
    ...
    def send_command(self, cmd: str) -> bool:  # todo test
        ...
        resp = self._req(
            url, json.dumps({"command": cmd, "channel_id": self.SELF_CHANNEL_ID})
        )  # '{"command":"' + cmd + '", "channel_id":'+str(self.SELF_CHANNEL_ID)+'}')
        ...
        if self.connected:
            ...
            try:
                ...
                if self.connection.open:
                    print(
                        "[TROVO WS CONN] Connection stablished. Client correcly connected"
                    )
        ...
        try:
            ...
            if (
                ...
                if t == 0:
                    ...
                    if (
                        msg["user"] == "NetTyan"
                    ):  # todo was in ctx.BotNicknames, solve another?
                        print(
                        ...
                        print(
                            f"[{datetime.now().strftime('%H:%M:%S')}] [TROVO CHAT]",
                            msg["user"],
                            ">",
                            msg["msg"],
                        )
        ...
        while True:
            try:
                if not (await self.check_connection()):
                    raise Exception(
                        "not connected to websocket after checking connecting"
                    )
                await self.connection.send(
                    json.dumps({"type": "PING", "nonce": "client-ping"})
                )
...
def trovo_client_thread(ctx_chat, trovo_actions_queue):
    trovo_thread = threading.Thread(
        target=run_trovo_client,
        args=(
            ctx_chat,
            trovo_actions_queue,
        ),
