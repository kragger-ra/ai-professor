...
from data_schema.chat_structures import NicknameData, UserData
...
def db_init(
    db: sl.Connection, cursor: sl.Cursor
...
"""
    ...
    cursor.execute(
        """
CREATE TABLE IF NOT EXISTS users_nicknames (
...
"""
    ...
    # четвертая таблица с переменными "УМА"
    cursor.execute(
        """
    CREATE TABLE IF NOT EXISTS mind_variables (
...
def init_mind_variable(db: sl.Connection, cursor: sl.Cursor):
    """Initialize mind variables if they don't exist

    Args:
        db (sl.Connection): Database connection
        cursor (sl.Cursor): Database cursor
    """
    cursor.execute("SELECT COUNT(*) FROM mind_variables")
    count = cursor.fetchone()[0]

    if count == 0:
        cursor.execute(
            """
INSERT INTO mind_variables (mind_name, mood, last_changed)
VALUES (?, ?, ?)
""",
            ("default", 0.0, eztime()),
        )
...
SQLITE_DB_FILEPATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))),
    "data",
    "NetTyanDatabase.db",
)

print("[SQLITE] Loading Main Database from " + SQLITE_DB_FILEPATH + "...")
...
class StreamerDatabase:
    def __init__(self):
        ...
        if safety_mode == 3:
            self.db_connection = sl.connect(SQLITE_DB_FILEPATH, check_same_thread=False)
        ...
        try:
            mood = self.get_mood()
            print("[DB WARMUP] MOOD = " + str(mood))
            if mood is None:
                raise sl.OperationalError("MOOD IS NONE")
        except sl.OperationalError as err:
            print(
                f"[DB INIT ERR] ОШИБКА ПРИ ПОДКЛЮЧЕНИИ К БАЗЕ ДАННЫХ! Путь к файлу: {SQLITE_DB_FILEPATH}",
                err,
            )
            db_init(self.db_connection, self.db_connection.cursor())
            init_mind_variable(self.db_connection, self.db_connection.cursor())
            print("[DB] RE-INIT (CREATED TABLES)")
            print("[DB WARMUP TRY 2] MOOD = " + str(self.get_mood()))
    ...
    def save_db_changes(self) -> bool:
        ...
        while fail and try_num < 10:
            ...
            try:
                ...
                print(
                    f"[DB SAVE ERR] ОШИБКА N={str(try_num)} ПРИ СОХРАНЕНИИ БАЗЫ ДАННЫХ! ",
                    err,
                )
    ...
    def get_db_field(
        ...
        try:
            cursor.execute(
                f"SELECT {field_name} FROM {table_name} WHERE {id_name} = ?",
                (field_id,),
            )
            if many:
    ...
    def set_db_field(
        ...
        try:
            cursor.execute(
                f"UPDATE {table_name} SET {field_name} = ? WHERE {id_name} = ?",
                (
                    field_new_value,
                    field_id,
                ),
    ...
    def set_mood(self, new_mood: float, mind_id: int = 1) -> bool:
        cursor = self.db_connection.cursor()
        if self.set_db_field(
            field_id=int(mind_id),
            field_name="mood",
            field_new_value=new_mood,
            table_name="mind_variables",
            id_name="mind_id",
            cursor=cursor,
        ):
            return self.set_db_field(
                field_id=int(mind_id),
                field_name="last_changed",
                field_new_value=eztime(),
                table_name="mind_variables",
                id_name="mind_id",
                cursor=cursor,
            )
    ...
    def set_user_last_question(self, user_id: int, question: str) -> bool:
        cursor = self.db_connection.cursor()
        return self.set_db_field(
            field_id=int(user_id),
            field_name="last_question_date",
            field_new_value=eztime(),
            cursor=cursor,
        ) and self.set_db_field(
            field_id=int(user_id),
            field_name="last_question",
            field_new_value=question,
            cursor=cursor,
        )

    def set_user_last_interact_time(
        self, user_id: int, new_value: str, last_answered=False
    ):
        if last_answered:
            return self.set_db_field(
                field_id=int(user_id),
                field_name="last_answered",
                field_new_value=new_value,
            )
        else:
            return self.set_db_field(
                field_id=int(user_id),
                field_name="last_interact",
                field_new_value=new_value,
            )
    ...
    def set_user_rank(self, user_id: int, new_rank: float):
        return self.set_db_field(
            field_id=int(user_id),
            field_name="rank",
            field_new_value=clamp(new_rank, 0.0, 10.0),
        )
    ...
    def get_or_create_user(
        self, data: dict, return_nick_id_too: bool = False
    ) -> Union[int, dict, None]:
        nick = data["user"]
        if nick.strip():
            ...
            if not return_nick_id_too:
                ...
                return {
                    "user_id": user_id,
                    "nick_id": self.get_user_nick_id(nick, data),
                }
        return None

    def get_user_nick_id(
        self, nick: str, data: dict = None, cursor: sl.Cursor = None
    ) -> int:
        if cursor is None:
            cursor = self.db_connection.cursor()
        if data is not None:
            ...
            if server and env:
                ...
                cursor.execute(
                    """SELECT nick_id FROM users_nicknames WHERE nick = (?) AND env = (?)""",
                    (nick, env),
                )

        else:
            cursor.execute(
                """SELECT nick_id FROM users_nicknames WHERE nick = (?)""", (nick,)
            )
        return self.get_cursor_result(cursor)

    def set_analyze_for_nick(self, nick_id: int, nick_analyze: str) -> bool:
        return self.set_db_field(
            field_id=int(nick_id),
            field_name="nick_analyze",
            field_new_value=nick_analyze,
            table_name="users_nicknames",
            id_name="nick_id",
        )
...
"""
        ...
        if inserted_nick_id is not None:
            self.save_db_changes()
            print(
                "[DB ADD NEW NICK TO USER ID("
                + str(user_id)
                + ") INSERTED "
                + nick
                + ". Nick in table ID = ("
                + str(inserted_nick_id)
                + ")"
            )
        return inserted_nick_id

    def add_new_user(self, data: dict = None) -> Union[int, None]:
        print("CALLED ADD NEW USER", data)
        if data == None:
...
f"""
        ...
        if inserted_id is not None:
            self.save_db_changes()
            print(
                "[DB ADD NEW USER] INSERTED "
                + name
                + " AT ID "
                + str(inserted_id)
                + " DEBUG TYPE "
                + str(type(inserted_id))
            )
            ...
            if data.get("env", "") == "discord":  # +дискорд ид к юзеру
                ...
                if discord_user_id:
                    ...
                    print(
                        f"[DB ADD DISCORD NICK] Добавлен для ника {str(name)} discord_id в дискорде"
                    )
    ...
    def add_diags(
        self, user_id: int, diag_to_add: list[dict], data: dict = None
    ) -> list[int]:
        ...
        if bind_to_nick_id == None and diag_nick:
            if diag_nick.strip() != "":
                bind_to_nick_id = self.get_user_nick_id(
                    nick=diag_nick, data=data, cursor=cursor
                )
        ...
        for i, record in enumerate(diag_to_add):
            ...
            if not filter_topics:
                filter_topics = None
            filter_results = record.get("filter_results", None)
            ...
            if date_rec is None:
                date_rec = date
            sql = f"""
INSERT INTO users_dialogs (user_id,diag_nick,content,role,date,command,emotion,env,server,other_info,bind_to_diag_id,bind_to_nick_id,filter_allowed,filter_topics,filter_results)
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
RETURNING diag_id
f"""
            cursor.execute(
                ...
                (
                    ...
                    record.get("command", None),
                    record.get("emotion", None),
                    # command,                   emotion,
                    env,
                    server,
                    other_info,
            ...
            if inserted_diag_id is not None:
                ...
                if bind_to_diag_id is not None:
                    cursor.execute(
                        "UPDATE users SET diags_count = diags_count + 1 WHERE user_id = ?",
                        (user_id,),
                    )
        if len(diag_ids) > 0:
            if last_interact_time:
                cursor.execute(
                    "UPDATE users SET last_answered = ? WHERE user_id = ?",
                    (
                        last_interact_time,
                        user_id,
                    ),
    ...
    def get_user_diag_count(self, user_id: int, real=True, cursor: sl.Cursor = None):
        ...
        if real:
            if self.check_user_id_exists(user_id):
                cursor.execute(
                    "SELECT COUNT(diag_id) FROM users_dialogs WHERE user_id = ? AND role = 'assistant'",
                    (user_id,),
                )
            else:
                return None
            ...
            cursor.execute(
                "SELECT diags_count FROM users WHERE user_id = ?", (user_id,)
            )
        return self.get_cursor_result(cursor)
...
LIMIT 1
    ...
    def get_relevant_diag(
        self,
        user_id: Union[int, None] = None,
        count: int = 1,
        exact_user_timeout: float = 160.0,
        any_user_timeout: float = 250.0,
    ) -> list[dict]:
        now = datetime.now()
        if user_id is not None:
            last_answered = self.get_db_field(
                field_id=user_id, field_name="last_answered"
            )
            if last_answered:
                try:
                    ...
                    print(
                        f"[DB Get Time EXACT relevant user_id={str(user_id)} diag ERR] err =",
                        err,
                    )
                    user_id = None
            else:
        ...
        if user_id is None:
            ...
            if last_answered:
                try:
                    if (now - tm(last_answered)).total_seconds() <= any_user_timeout:
                        print(
                            "[DB get ANY RELEVANT DIAG] time succed, searching for ANY diag.."
                        )
            ...
            print(
                f"[DB get EXACT user_id={str(user_id)} RELEVANT] time succed, searching for EXACT diag.."
            )
            diags = self.get_user_diags(user_id=user_id, count=count)
        return diags

    def get_user_summary(self, user_id: int) -> str:
        """Get the summary text for a user

        Args:
            user_id (int): The user ID to get summary for

        Returns:
            str: The user's summary text, or None if not set
        """
        return self.get_db_field(field_id=int(user_id), field_name="summary")

    def set_user_summary(self, user_id: int, summary: str) -> bool:
        """Set the summary text for a user

        Args:
            user_id (int): The user ID to set summary for
            summary (str): The summary text to set

        Returns:
            bool: True if successful, False otherwise
        """
        return self.set_db_field(
            field_id=int(user_id), field_name="summary", field_new_value=summary
        )

    def get_user_data(self, user_name: str, raise_errors: bool = False) -> UserData:
        """Get comprehensive user data including nicknames and metadata

        Args:
            user_name (str): The username to lookup

        Returns:
            UserData: User data structured according to UserData type
        """
        try:
            cursor = self.db_connection.cursor()
            ...
            cursor.execute(
                """SELECT name, rank, last_interact, last_answered, summary 
            ...
            cursor.execute(
                """SELECT nick, env, server, nick_analyze, added_date  
                FROM users_nicknames WHERE user_id = ?""",
                (user_id,),
            )
            nickname_rows = cursor.fetchall()

            # Format user's nicknames
            nicknames = {}
            for row in nickname_rows:
                nick, env, server, nick_analyze, added_date = row
                nickname_data: NicknameData = {
                    "nick": nick,
                    "env": env,
                }
                ...
                if server:
                    nickname_data["server"] = server
                if nick_analyze:
                    nickname_data["nick_analyze"] = nick_analyze
                if added_date:
                    nickname_data["added_date"] = added_date

                # Use nickname as the key
                nicknames[nick] = nickname_data

            # Get the most recent activity timestamp
            last_activity = user_row[2]  # last_interact
            if user_row[3] and user_row[3] > last_activity:  # last_answered
                last_activity = user_row[3]

            # Format the complete UserData object
            user_data: UserData = {
                "name": user_name,
                "user_id": user_id,
                "found_count": 1,
                ...
                "last_activity_string": last_activity if last_activity else "",
                "binded_events": [],
                "last_db_interaction": user_row[2],  # last_interact
                "summary": user_row[4] if user_row[4] else "",
                "rank": float(user_row[1]) if user_row[1] is not None else 0.0,
                "nicknames": nicknames,
            }
        except Exception as e:
            print(f"[DATABASE] Error getting user data for {user_name}: {e}")
            if raise_errors:
                raise e
            return {}
        return user_data
...
if __name__ == "__main__":
    db = StreamerDatabase()
    db.get_user_data("Ошибка")
    exit()
    # Test new user creation with summary
    debug_user = {
        "user": "TestUser123",
        "env": "minecraft",
        "server": "test.server.com",
    }
    user_id = db.get_or_create_user(debug_user)
    if user_id:
        db.set_user_summary(
            user_id, "This is a test user created to validate the new summary field."
        )
        print(f"Created test user with ID {user_id}")
        print(f"User summary: {db.get_user_summary(user_id)}")

    # Test dialog with filter results
    debug_dialog = [
        {
            "user": "TestUser123",
            "role": "user",
            ...
            "emotion": "",
            "command": "",
            "filter_results": '{"toxicity": 0.1, "sentiment": "positive"}',
        },
        {
            "user": "default",
            "role": "assistant",
            "content": "I'm doing great! Nice to meet you!",
            "date": eztime(),
            "emotion": "happy",
            "command": "",
            "filter_results": '{"response_quality": 0.95}',
        },
    ]

    if user_id:
        diag_ids = db.add_diags(user_id=user_id, diag_to_add=debug_dialog)
        print(f"Added test dialogs with IDs: {diag_ids}")
    ...
    debug_dialog = [
        {
            "user": "LexaLepexa",
            "role": "user",
            "content": "2Привет! Как дела?",
            "date": "2023-06-30 20:22:59",
            "emotion": "",
            "command": "",
        },
        {
            "user": "default",
            "role": "assistant",
            "content": "3Пока! Ты скучный!",
            "date": "2023-06-30 20:23:00",
            "emotion": "агрессия",
            "command": "бан",
        },
        {
        ...
        },
        {
            "user": "default",
            "role": "assistant",
            "content": "5Не эээкай нищщщ",
            "date": "2023-06-30 20:25:00",
            "emotion": "агрессия",
            "command": "бан",
        },
    ]
